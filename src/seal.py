...
if __name__ == "__main__":
    ...
    args.res_dir = os.path.join(
        ...
        f'{args.dataset.split("-")[1]}+{time.strftime("%m%d%H%M%S")}',
    )
    ...
    if dataset.eval_metric.startswith("hits@"):
        dataset.eval_metric = f"hits@{args.eval_hits_K[0]}"
    ...
    if args.use_warmup:
        ...
        node_attribute = torch.load(f'{dataset_path}/warmup_embedding_{args.warmup_core}.pt', map_location = 'cpu')
        graph.ndata['feat'] = node_attribute
